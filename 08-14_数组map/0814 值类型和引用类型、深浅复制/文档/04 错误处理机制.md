...
JavaScript 解析或运行时，一旦发生错误，引擎就会抛出一个错误对象，然后抛出这个错误对象，导致程序终止（崩溃，死亡）。
...
// 创建一个err并不会导致程序终止，只有throw err才会导致程序终止
// throw 终止程序运行，抛出错误
throw err;
```
...
function catchit() {
  try {
    ...
    console.log(err.stack); // 打印stack
  }
}

catchit()
//Error
//    at throwit (错误对象.html:24)
//    at catchit (错误对象.html:29)
//    at 错误对象.html:35
```
...
```js
// 使用一个不存在的变量
console.log(tom);	// Uncaught ReferenceError: tom is not defined
```
...
```js
var tom;
console.log(tom.name);// TypeError: Cannot read property 'name' of undefined

var tom = null;
console.log(tom.name);// TypeError: Cannot read property 'name' of null

new 123	// Uncaught TypeError: 123 is not a constructor

var obj = {};
obj.run()	// Uncaught TypeError: obj.run is not a function
```
...
```js
try { 
	// try中执行的代码，可能有异常。一旦发现异常，则立即跳到catch执行。否则不会执行catch里面的内容 
} 
catch (error) { 
...
上面代码中，如果函数`tom.sex`执行报错，就会进行`catch`代码块，接着对错误进行处理。
...
// 完成清理工作
// Uncaught Error: 出错了……
//    at cleansUp (try...catch...finally.html:3:11)
//    at try...catch...finally.html:10:1
```
...
try {
  ...
  //finally中的代码一定会执行，这以便让你有机会能够在异常处理最后做一些清理工作。如关闭数据库连接等等
  console.log('我是清理工')
}
// 如果catch中没有抛出错误，finally执行后会继续执行后面的代码
// 如果catch中抛出错误，会执行catch，然后执行finally，finally之后的代码是否会执行取决于catch中是否抛出新的错误

console.log('---可以执行到这里吗？----');
```

`try...catch...finally`不能大量使用：会不停的扫描、清理栈，影响性能。

一般不建议乱用`try...catch...finally`语句。一般只有人为无法判断代码是否正常运行时，才会用try...catch... 让此语句帮你智能判断业务逻辑是否正常运行。